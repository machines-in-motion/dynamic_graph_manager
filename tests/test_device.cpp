...
TEST_F(TestDevice, test_set_sensors_from_map)
{
    ...
    ASSERT_TRUE(
        sensors["imu"].isApprox(device.sensors_out_["imu"]->accessCopy()));
}
...
public:
    ...
    SimpleRobot(std::string RobotName) : Device(RobotName)
    {
        initialize(YAML::LoadFile(TEST_CONFIG_PATH +
...
TEST_F(TestDevice, test_get_controls_to_map)
{
    ...
    ASSERT_TRUE(
        torques.isApprox(device.motor_controls_in_["torques"]->accessCopy()));
