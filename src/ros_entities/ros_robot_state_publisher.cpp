...
#include "dynamic_graph_manager/ros_entities/ros_robot_state_publisher.hpp"
#include <tf/tf.h>
#include <iostream>
                                 ...
                                 const std::string& joint_state_topic_name)
{
    ...
    pub.callback_function_ = boost::bind(&RosRobotStatePublisher::send_data,
                                         this,
...
void RosRobotStatePublisher::send_data(
    const RosRobotStatePublisherInternal& pub, int time)
{
    // some shortcuts
    ...
    if (pub.joint_state_publisher_->trylock())
    {
        ...
        for (unsigned int i = 0;
             i < pub.joint_state_publisher_->msg_.position.size();
             ++i)
        {
            pub.joint_state_publisher_->msg_.position[i] =
                robot_state(index + i);
        }
