/**
 * @file ros_robot_state_publisher_mt.cpp
...
RosRobotStatePublisherMt::RosRobotStatePublisherMt(const std::string& name)
    : Entity(name),
      trigger_signal_(
          boost::bind(&RosRobotStatePublisherMt::trigger, this, _1, _2),
...
void RosRobotStatePublisherMt::add(const std::string& base_link_name,
                                   ...
                                   const std::string& joint_state_topic_name)
{
    ...
    publishers_[joint_state_topic_name] =
        std::make_shared<RosRobotStatePublisherMtInternal>();
    // create shortcuts
    RosRobotStatePublisherMtInternal& pub =
        *publishers_[joint_state_topic_name];
    ...
    pub.base_tf_publisher_ =
        std::make_shared<TfRtPublisher>(ros_node_handle_, "/tf", 1);
    ...
    oss_signal_name << "RosRobotStatePublisherMt(" << this->name << ")::"
                    << "input(vector" << joint_names_list.size()
                    << "d)::" << signal_name;
    ...
    pub.callback_function_ =
        boost::bind(&RosRobotStatePublisherMt::copy_data_internally,
                    this,
                    publishers_[joint_state_topic_name],
                    _1);

    // initialize the publishing loop
    pub.spinner_.set_period(0.01);

    // l;aunch the publishing thread
    pub.thread_ = std::thread(&RosRobotStatePublisherMt::send_data,
                              this,
                              std::ref(*publishers_[joint_state_topic_name]));
}

int& RosRobotStatePublisherMt::trigger(int& dummy, int dg_time)
{
    // We call all the callback function: (send_data) corresponding to the
    // registered topic to send
    typedef std::map<
        std::string,
        std::shared_ptr<RosRobotStatePublisherMtInternal>>::iterator PubItType;
    ...
    for (PubItType pub_it = publishers_.begin(); pub_it != publishers_.end();
         ++pub_it)
    {
        pub_it->second->callback_function_(dg_time);
    }

    return dummy;
}

void RosRobotStatePublisherMt::copy_data_internally(
    std::shared_ptr<RosRobotStatePublisherMtInternal> publisher, int dg_time)
{
    // acquiere the input signal
    publisher->mutex_.lock();
    publisher->robot_state_ =
        publisher->robot_state_input_signal_->access(dg_time);
    publisher->mutex_.unlock();
}

void RosRobotStatePublisherMt::send_data(
    RosRobotStatePublisherMtInternal& publisher)
{
    // some shortcuts
    geometry_msgs::TransformStamped& base_pose_msg =
        publisher.base_tf_publisher_->msg_.transforms[0];
    sensor_msgs::JointState& joint_state_msg =
        publisher.joint_state_publisher_->msg_;
    // local copy of the data
    dynamicgraph::Vector robot_state;
    // update the init ros just in case
    ros_init(DynamicGraphManager::dg_ros_node_name_);

    publisher.stop_publish_ = false;
    while (!publisher.stop_publish_)
    {
        // time the loop with the spinner
        publisher.spinner_.spin();

        // acquiere the input signal
        publisher.mutex_.lock();
        robot_state = publisher.robot_state_;
        publisher.mutex_.unlock();
        ...
        assert((robot_state.size() == joint_state_size ||
               ...
               "Input signal has the wrong size. The expected size is: number "
               "joint, or"
        ...
        {
            if (publisher.base_tf_publisher_->trylock())
            {
                ...
                publisher.base_tf_publisher_->unlockAndPublish();
            }
            index += 7;
        ...
        {
            publisher.joint_state_publisher_->msg_.header.stamp =
                ros::Time::now();
            for (unsigned int i = 0;
                 i < publisher.joint_state_publisher_->msg_.position.size();
                 ++i)
            {
                publisher.joint_state_publisher_->msg_.position[i] =
                    robot_state(index + i);
            }
            publisher.joint_state_publisher_->unlockAndPublish();
        }
    }  // end while(!publisher.stop_publish)
}

void RosRobotStatePublisherMt::set_tf_msg_to_identity(
...
void RosRobotStatePublisherMt::normalize_tf_msg_quaternion(
    geometry_msgs::TransformStamped& msg)
{
